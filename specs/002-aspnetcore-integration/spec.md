...
- What happens when a non-seekable stream is passed — `PndArchive.Open` reads and sets `stream.Position` during archive-type detection, so non-seekable streams throw `NotSupportedException`. The factory's `Open` propagates it; `TryOpen` catches it and returns `false`. Callers are expected to pass a seekable stream (e.g. `FileStream` or `MemoryStream`).
